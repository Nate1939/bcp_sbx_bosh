...
bosh -e $BCP_SELECTED_DIRECTOR update-cpi-config --no-redact deployments/$BCP_SELECTED_DIRECTOR/cpi-config/puzzel-cpis.yml \
    ...
    -v az_euw_dev_subscription_id="$(az keyvault secret show --vault-name "bosh-prod-uks-kv" --name "azure-euw-dev-subsription-id" --query "value" -o tsv)" \
    -v az_euw_dev_tenant_id="$(az keyvault secret show --vault-name "bosh-prod-uks-kv" --name "azure-euw-dev-tenant-id" --query "value" -o tsv)" \
    -v az_euw_dev_client_id="$(az keyvault secret show --vault-name "bosh-prod-uks-kv" --name "azure-euw-dev-client-id" --query "value" -o tsv)" \
    -v az_euw_dev_client_secret="$(az keyvault secret show --vault-name "bosh-prod-uks-kv" --name "azure-euw-dev-client-secret" --query "value" -o tsv)" \
